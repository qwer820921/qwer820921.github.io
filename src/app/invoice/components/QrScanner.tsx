...
const QrScanner: React.FC = () => {
  ...
  const handleScanSuccess = (decodedText: string) => {
      ...
      if (result.isWinner) {
        playWinSound(); // 播放中獎音效
        toast.success(`🎉 中獎了！${invoiceData.period} ${invoiceData.number} - ${result.prize?.name} ${result.prize?.amount ? `$${result.prize.amount}` : ''}`);
      } else {
      ...
      {!loading && !error && hasCameraPermission !== false && (
        <>
          ...
          {scannedList.length > 0 && (
            <div className="mt-4">
              <h3 className="text-white text-lg mb-3 border-bottom pb-2">掃描紀錄</h3>
                        ...
                        {record.result.isWinner ? (
                          <>
                            ...
                            <div className="text-warning fw-bold">
                                {record.result.prize?.amount ? `$${record.result.prize.amount}` : '金額依實際領獎'}
                            </div>
                          </>
                        ) : (
