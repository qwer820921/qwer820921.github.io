  ...
  (
    ...
    const togglePlay = useCallback(() => {
      if (isPlaying) {
        ...
        if (visibleIdx !== -1) {
          const currentActiveEl = document.getElementById(
            `tts-node-${currentIndex}`
          );
          if (currentActiveEl) {
    ...
    const handleVoiceChange = useCallback((newVoiceURI: string) => {
      voiceChangeRef.current = true;
      setSelectedVoiceURI(newVoiceURI);
    }, []);

    // 當語音改變且正在播放時，用新語音重新讀當前段落
    useEffect(() => {
      if (voiceChangeRef.current && isPlaying) {
    ...
    return (
      <>
        ...
        {/* 底部彈窗 */}
        <div
          className={`${styles.ttsDrawer} ${isOpen ? styles.ttsDrawerOpen : ""}`}
        >
          {/* 拖曳提示條 */}
