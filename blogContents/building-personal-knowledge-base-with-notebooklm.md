...
實際把技術文件上傳到 NotebookLM 時，第一個遇到的問題是 PDF 的文字品質。掃描版的 PDF（尤其是舊版 RFC 文件或學術論文）解析出來的文字充斥著亂碼，NotebookLM 的回答品質就會很差。解決方法是先用 Adobe Acrobat 或免費的 online OCR 工具重新轉換，確保 PDF 的文字層是可搜尋的，再上傳效果才好。

每個 Notebook 的來源限制是 50 個文件，每個文件 500,000 tokens 上限。對於中文技術文件來說，大約等於 30-40 萬個中文字，一本中等長度的技術書籍就快到上限了。因此在組織 Notebook 的時候，建議按照「主題」而不是「文件種類」來拆分，例如把所有「認證與安全」相關的文件放在同一個 Notebook，這樣問問題時 AI 能跨文件整合，回答品質更好。

Audio Overview 功能（把知識庫轉成 Podcast 對話）非常適合在通勤時聆聽複習。試過把五篇 LLM 相關論文摘要轉成對話，兩個 AI 主持人討論的方式比直接讀摘要更容易記住重點。只是中文文件有時候會生成英文 podcast，這點還不太穩定。
