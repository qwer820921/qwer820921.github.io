...
在實際審查過幾個專案的 JWT 實作之後，最常見的安全漏洞不是演算法選擇，而是**驗證步驟不完整**。很多開發者只驗證簽章有效性，卻跳過了 `iss`（發行者）和 `aud`（受眾）的比對。這在單服務系統裡可能問題不大，但在微服務架構中，一個服務簽發的 Token 如果沒有 `aud` 限制，理論上可以被拿去存取任何其他服務，形成橫向移動漏洞。

localStorage 與 httpOnly Cookie 的選擇爭論了很久。localStorage 對 XSS 攻擊是開放的，httpOnly Cookie 則對 CSRF 敏感但搭配 SameSite=Strict 可以緩解。我們團隊最後的決策是：所有需要身份驗證的 API 一律用 httpOnly Cookie 傳遞 JWT，前端完全感知不到 Token 的存在。這讓前端程式碼簡單很多，也消除了 `Authorization: Bearer` header 需要手動附加的繁瑣。

`none` 演算法漏洞第一次聽到時覺得不可思議——怎麼會有系統接受沒有簽章的 JWT？但仔細想想，如果 JWT 驗證函式庫的預設行為是「信任 Header 中宣告的演算法」，那攻擊者只需要把 `alg` 改成 `none` 就能繞過驗證。這個漏洞提醒我：永遠在伺服器端明確指定允許的演算法清單，不要讓客戶端自行聲明。
