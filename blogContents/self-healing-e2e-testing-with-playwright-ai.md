...
實際導入自癒機制時，遇到的最大問題是 `page.accessibility.snapshot()` 回傳的資料品質參差不齊。有些頁面的 Accessibility Tree 非常完整，AI 可以精準找到對應元素；但某些用 div 硬幹的舊頁面，拿到的 snapshot 幾乎是空的，LLM 只能靠 DOM snapshot 猜測，準確率大幅下降。後來學到的教訓是：先補足 `aria-label` 和 `role` 屬性，自癒成功率才能真正提高，自癒機制是手段，語意化 HTML 才是根本。

CI 環境的延遲也是個坑。在本機測試時，AI 建議的新 Selector 幾乎都能在第一次重試時成功；但在 CI 的 headless 環境裡，頁面有時候還沒完全渲染完就開始截圖，LLM 拿到的是半空的 DOM，建議品質就差很多。後來在 interceptor 裡加了 `page.waitForLoadState('networkidle')` 再截圖，問題改善了很多。

成本控制方面，每次測試失敗都呼叫 GPT-4o 其實蠻貴的。建議設計一個「快取層」，如果同一個 Selector 在過去 7 天內已經被自癒過，直接用上次的結果而不重新呼叫 LLM。這樣可以把 80% 的重複性失敗的 API 費用省掉。
