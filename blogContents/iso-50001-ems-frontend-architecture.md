...
能源管理系統的資料更新頻率設計和一般業務系統有本質差異。一般 SaaS 平台可能幾秒更新一次，但 ISO 50001 的 EnPI 數據通常是每 15 分鐘或每小時彙總一次來自感測器的原始數據。這個節奏決定了前端的更新策略：不需要 WebSocket 實時推送，用 React Query 的 `refetchInterval` 設定成 5 分鐘輪詢就夠了，反而讓實作變得更簡單、後端壓力也更小。

稽核軌跡（Audit Trail）的 UI 設計比預期困難。數據本身簡單，但審核者需要在同一個畫面上同時看到「是誰、在什麼時間、改了什麼、從什麼改到什麼」，還要能篩選特定時間段。第一版設計成表格，結果欄位太多擠在一起很難看。後來改成 Timeline 式的呈現，每一筆變更是一個卡片，配合折疊展開顯示詳細 diff，審查員的滿意度明顯提升。

離線支援的部分用 Service Worker + IndexedDB 實作，但有一個容易被忽略的邊界情況：使用者在離線狀態下修改了檢核表，回到線上後如果後端的資料也被別人修改了，衝突解決的 UI 很難做得好。最後的解決方案是採取「後者優先」策略，但在同步前強制顯示 diff 確認視窗，讓使用者自己決定要保留哪個版本。
