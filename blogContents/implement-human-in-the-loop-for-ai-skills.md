...
Human-in-the-loop 在設計上最難的不是技術實作，而是「決定哪些決策要求人工審核」這個判斷本身。一開始把所有「寫入型」操作都加上審核，結果審核通知量暴增，審核者很快就開始無腦按「批准」，HITL 機制形同虛設。後來改成「影響金額超過閾值」或「修改超過 N 筆記錄」才觸發審核，通知量降到原來的 1/5，審核者反而開始認真看每一筆了。

審核超時的處理策略也值得深思。設定「超時自動拒絕」在大多數場景是對的，但有些緊急操作如果卡在等審核、超時又被拒絕，可能造成業務損失。後來設計了兩層：一般操作超時 30 分鐘自動拒絕；緊急操作標記「高優先級」並同時通知多位審核者，任一人批准即可執行，且超時時間縮短為 5 分鐘並持續發送提醒。

通知格式影響了審核效率。一開始只傳了「有一個操作需要審核」，審核者還要另外登入系統查詳情，點擊率很低。後來直接在通知訊息裡帶上「操作摘要、影響範圍、批准/拒絕連結」，一條訊息就能完成審核，效率大幅提升。這個細節讓我理解：HITL 設計的重點不只是「要不要讓人插手」，還有「讓人插手的成本」夠不夠低。
