...
這篇文章的觀點在我自己的日常工作流中得到了很直觀的驗證。一年前，我處理資料的方式還是：打開 Excel、手動下載 CSV、貼到試算表裡算。現在的方式是：告訴 AI 我要分析什麼，AI 直接呼叫 API 拿資料回來，跑完分析後生成報告。整個過程裡，我幾乎沒有碰任何 GUI，全程都是「對話 → API → 結果」。

從開發者角度來看，這個轉變讓我重新評估哪些前端工作真的值得投入。如果一個功能的使用者有 80% 是透過 AI Agent 呼叫的，那花時間做精美的 UI 動畫意義不大，反而應該把精力放在 API 的 schema 設計、錯誤訊息的可讀性、以及 MCP Tool 的 description 寫得夠不夠清楚。這是一個資源分配思維的根本轉變。

不過「介面消失」並非全面的。有些體驗確實需要視覺化 UI——複雜的資料儀表板、設計工具、遊戲——這些場景下 AI 無法取代人眼和直覺判斷。真正會消失的是那些「只是把資料搬來搬去」的 UI，這類工作 AI Agent 確實做得比人類既快又便宜。
