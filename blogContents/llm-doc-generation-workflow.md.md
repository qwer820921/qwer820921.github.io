...
把 LLM 文件生成工作流套進真實的 codebase 後，最大的發現是：**程式碼品質直接決定了生成文件的品質**。變數命名模糊（例如 `data`、`result`、`temp`）的函數，生成出來的文件幾乎沒有意義，LLM 只能照著字面重述，完全沒有「解釋為什麼」的能力。這個問題反過來成了一個強迫重構的動力——為了讓文件生成結果好看，要先把命名整理清楚。

分塊策略（chunking strategy）的選擇比我預期的重要。一開始以整個檔案為單位送給 LLM，結果大型服務類別的文件要塞進非常多 context，品質反而不穩定。改成以函數為單位分塊後，每次 LLM 專注在單一函數，描述的準確度明顯提升。但這帶來另一個問題：函數之間的關係和呼叫鏈在文件裡消失了，需要額外的「整合摘要」步驟把個別函數文件串接成模組級別的說明。

最後定型的流程是：固定的 MDX 模板結構（Overview、Parameters、Return Value、Examples、Notes）+ 嚴格的 JSON Schema 輸出格式驗證 + CI 觸發。有了固定結構，文件之間的一致性大幅提升，查閱時也更直覺。最意外的收穫是：CI 整合後，每次 PR 都會自動更新對應函數的文件，「文件跟不上程式碼」的問題在這個 repo 上幾乎消失了。

---

**參考資料**

- [1] DEV Community. (2025, August 29). _Optimizing technical documentations for LLMs_. Retrieved from https://dev.to/joshtom/optimizing-technical-documentations-for-llms-4bcd
- [2] Medium. (2026, February 8). _5 LLM Workflow Patterns for Building Scalable AI Applications_. Retrieved from https://medium.com/@yadavdivy296/5-llm-workflow-patterns-for-building-scalable-ai-applications-a-complete-guide-376b7d7ccd1b
- [3] Addy Osmani. (2025, December 18). _My LLM coding workflow going into 2026_. Retrieved from https://addyo.substack.com/p/my-llm-coding-workflow-going-into
- [4] Towards AI. (2026, February 6). _Build LLM-Powered Documentation that Always Stays True to latest Codebases_. Retrieved from https://towardsai.net/p/machine-learning/build-llm-powered-documentation-that-always-stays-true-to-latest-codebeases
