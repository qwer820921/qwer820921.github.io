...
Guardrails 的 false positive 率調整是最耗時的部分。第一版的關鍵字黑名單設定得很嚴格，結果合法的技術文章摘要請求（裡面提到「忽略指令」、「系統提示」等詞彙）也被攔截了，使用者不斷收到拒絕訊息，體驗很差。後來改成「關鍵字 + 語境」雙重判斷，即同時出現兩個以上的注入特徵詞，再加上確認是在 user input 裡而不是 system context 裡，誤報率才降到可接受範圍。

輸出過濾層的設計比輸入過濾層更微妙。輸入可以做關鍵字掃描，但輸出是 LLM 生成的自然語言，很難用規則完整覆蓋。實際遇到過 LLM 被注入後，用「比喻」的方式洩漏 system prompt（例如「就像某個祕密食譜」），純文字比對根本偵測不到。最終解法是在輸出層加一個「自我評估」步驟，用另一個 LLM 判斷輸出是否包含不應公開的資訊，雖然增加了 latency，但對高風險場景來說值得。

防注入永遠是個軍備競賽。Guardrails 的規則和黑名單需要持續更新，建議定期做紅隊測試（Red Teaming），讓同事嘗試繞過現有的 Guardrails，再根據結果補強弱點。
