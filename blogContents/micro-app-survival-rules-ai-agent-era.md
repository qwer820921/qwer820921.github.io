...
把這套思維套用到自己的網站工具時，最有感的改變是重新思考每個工具的 API 介面設計。原本的發票對獎、QR Code 生成器都是純前端操作，沒有任何 API 端點。但如果要讓 AI Agent 能呼叫這些工具，就需要把核心邏輯抽出來變成可獨立呼叫的函數，並暴露成 MCP Tool 或 REST endpoint。這個重構過程意外地讓程式碼品質變好了——因為被迫把「UI 邏輯」和「業務邏輯」清楚分離。

「語意化錯誤訊息」這個概念很有啟發性。測試過讓 AI 自動操作投資工具時，如果 API 只回傳 `{"error": "400 Bad Request"}`，AI 完全不知道下一步怎麼做；但如果回傳 `{"error": "查詢日期超出範圍，台股資料最早可查 2010-01-01"}` ，AI 會自動調整參數重試。這個差異讓我開始重新設計所有工具的 error response 格式。

80% App 消失的預言或許過於激進，但「把你的工具做成 AI 能直接呼叫的 API」這個方向是確定的。這不需要馬上全部重寫，可以從最常用的功能開始，一個一個加上 MCP Tool 定義，逐步建立自己的 AI 工具庫。
