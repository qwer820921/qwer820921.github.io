---
title: "【AI 驅動的台股自動化助手】結合 OpenClaw 建立每日強勢股監控系統"
date: "2026-02-13"
author: "子yee"
description: "運用 OpenClaw 的瀏覽器控制與排程功能，打造一套自動抓取台股數據、篩選強勢股並生成報告的 AI 投資助手。"
category: "AI Agent"
tags:
  [
    "OpenClaw",
...
最先踩到的坑是時區問題。台股交易時間是台灣時間 9:00–13:30，但排程系統預設 UTC，差了八個小時。初次設定時把收盤後的爬蟲排在了 UTC 13:30，實際觸發時台灣市場早已關閉超過兩小時，抓到的數據全是靜態收盤價，沒有意義。之後統一在 cron 表達式上標注時區，並在日誌中記錄台灣時間作為參考。

第二個問題是不同數據源的資料品質差異。Yahoo 股市的數據延遲約 15 分鐘，玩股網的即時性較好但 DOM 結構更不穩定——偶爾改版就讓爬蟲全面失效。後來的架構是以玩股網作為主要來源、Yahoo 作為備援，並在 Skill 回傳結果中加上 `data_source` 欄位，讓 AI 知道資料的即時性，從而調整分析措辭的確定性。

最讓我警覺的是 AI 生成的「強勢股分析」如果沒有明確的免責聲明，讀起來很像真實的投資建議。系統最後在每份報告末尾都自動附加一段聲明：「以上為技術指標分析，不構成投資建議。」這不只是法律考量，也是對使用者認知的保護。
