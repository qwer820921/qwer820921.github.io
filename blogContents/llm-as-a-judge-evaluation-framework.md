...
設計 Rubrics 時最難拿捏的是「粒度」。一開始我把評分標準拆得很細，每個維度都有 5 個分數層級和詳細說明，結果 judge 模型反而容易陷入「分析癱瘓」，對邊界案例的判斷變得不一致。後來改成 3 個層級（1/3/5 分），每個層級只給一句話描述，judge 的穩定性反而提高了。Rubrics 不是越詳細越好，要讓 LLM 能快速、一致地對號入座。

在選擇 judge 模型時做了一個有趣的實驗：同一批回答，分別用 GPT-4o 和 Claude 3.5 Sonnet 當 judge，發現兩個模型的評分傾向有明顯差異，Claude 在「安全性和拒絕有害請求」的維度上會給更嚴格的分數，GPT-4o 在「流暢度」上評分較寬鬆。後來改成雙 judge 模式，兩個模型都跑一遍，分數差超過 2 的才標記為「需要人工審核」，整體評測可信度提高了很多。

另外一個實際踩坑：LLM-as-a-Judge 對 prompt 的長度很敏感。被評測的回答如果超過 2000 tokens，judge 模型的注意力會開始分散，評分品質明顯下降。加了一個前處理步驟，把超長回答先摘要再送去評測，品質就穩定多了。
