...
從 HS256 遷移到 RS256 的最大心理障礙是放棄「共享密鑰」的直覺。HS256 的對稱設計簡單直觀，但在微服務架構下，每個服務都持有同一把 secret key 讓我非常不安——任何一個服務被滲透，整套系統的 Token 都可以被偽造。切換到 RS256 後，私鑰只存在於 Auth Service，其他服務只持有公鑰，安全隔離的感覺立刻好多了。

JWKS 端點的快取策略讓我糾結了一段時間。公鑰理論上可以長期快取（幾小時甚至更長），但 key rotation 時如果快取還沒過期，就會出現「驗證失敗但 Token 其實有效」的問題。最後採用的策略是：先用快取的公鑰驗證，失敗時才主動重新拉取 JWKS，並在 Header 中的 `kid` 不在本地緩存時立即觸發更新。這個「懶惰更新」策略兼顧了效能和正確性。

真正理解 JWKS 的「頓悟時刻」是發現 JWT Header 中的 `kid`（Key ID）欄位的用途。原本以為 JWKS 只是一個公鑰清單，後來才明白 `kid` 讓客戶端能精確找到對應的驗證金鑰，不需要逐一嘗試。這個設計使得 key rotation 可以做到無縫切換——新舊金鑰可以同時存在於 JWKS 中，讓不同時期簽發的 Token 都能被正確驗證。
