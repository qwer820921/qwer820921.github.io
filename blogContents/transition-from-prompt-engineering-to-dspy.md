...
從手動 prompt engineering 轉向 DSPy 的過程，最大的心態調整是「放棄對 prompt 文字的執著」。原本花了很多時間精雕細琢 system prompt 的每一個字，結果 DSPy 的 teleprompter 優化出來的 prompt 文字看起來很「醜」，又長又不像人寫的，但效果卻更好。這個反直覺的結果讓我理解：對 LLM 有效的 prompt，不一定是對人類易讀的 prompt。

DSPy 的學習曲線比預期陡。文件中的概念（Signature、Module、Teleprompter）各自清楚，但要把三者串起來應用到真實 pipeline，需要一段摸索期。`dspy.Predict` 和 `dspy.ChainOfThought` 的使用時機是個很常見的誤區——簡單的分類任務用 `Predict` 就好，強行用 CoT 反而讓模型多產出一堆不必要的推理過程，token 費用上升但準確率沒提升。

最大的收穫是自動評估框架強迫你把「什麼叫做好的回答」具體定義出來。在手動調 prompt 的時候，很多判斷靠感覺；改用 DSPy 之後，必須寫出可量化的 metric function，這個過程本身就讓我對任務需求的理解更清晰了。
